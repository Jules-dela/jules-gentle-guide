...
-- 2. Update sign_contract to also persist hash
...
BEGIN
  ...
  VALUES (
    ...
    p_contract_data->>'signature_hash'
  );
  ...
  SET contract_data = jsonb_build_object(
    ...
    'signature_hash', p_contract_data->>'signature_hash'
  ),
  status = v_new_status
  WHERE id = p_case_id;
END;
...
-- 3. Helpful index for lookups by email + status
CREATE INDEX IF NOT EXISTS idx_intake_submissions_email_lower
  ON public.intake_submissions ((lower(email)));
CREATE INDEX IF NOT EXISTS idx_intake_submissions_session
  ON public.intake_submissions (stripe_session_id);